...
set :domain, '101.132.237.36'
set :deploy_to, '/home/server/puhuishequ'
set :repository, 'https://git.coding.net/niucang/homeland.git'
set :branch, 'puhuishequ'

# Optional settings:
set :user, 'puhui'          # Username in the server to SSH to.
...
set :shared_files, fetch(:shared_files, []).push('config/config.yml', 'config/database.yml', 'config/redis.yml', 'config/application.yml', 'config/secrets.yml')
...
task :setup do
  ...
  command %[echo "-----> Be sure to edit '#{fetch(:shared_path)}/config/config.yml'"]

  command %[touch "#{fetch(:shared_path)}/config/redis.yml"]
  command %[echo "-----> Be sure to edit '#{fetch(:shared_path)}/config/redis.yml'"]
end
